...
class SFTJsonl(Dataset):
    def __init__(self, path, tok, max_len=1536):
    ...
    def __getitem__(self, i):
        ...
        # Train the model to produce *full CoT + final line*
        target = f"{ex['rationale'].rstrip()}\n#### {ex['answer']}".rstrip()
        # Optionally end with EOS so generation knows when to stop
        target = target + self.tok.eos_token
...
class LeftPadCausalCollator:
    ...
    def __call__(self, features: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        if self.pad_to_multiple_of:
            ...
            if r: max_len += (self.pad_to_multiple_of - r)
...
def main():
    ...
    ap.add_argument("--train_jsonl", default="Data/star_train_from_base.jsonl")
    ap.add_argument("--out_dir", default="Models/star_sft")
    ap.add_argument("--per_device_batch", type=int, default=1)
    ap.add_argument("--grad_accum", type=int, default=48)
    ...
    assert Path(args.train_jsonl).exists(), args.train_jsonl
    ...
    targs = TrainingArguments(
        ...
        max_grad_norm=1.0,
        logging_strategy="steps", logging_steps=20, save_strategy="epoch",
        disable_tqdm=False, bf16=torch.cuda.is_available(), gradient_checkpointing=True,
        optim="adamw_torch", report_to=[], group_by_length=True,
    )
